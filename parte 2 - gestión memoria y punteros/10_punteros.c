/*
 * 10_punteros.c - Parte 2, Lección 10: Punteros
 *
 ...
 * posiblemente la característica más potente y singular del lenguaje C.
 *
 * Idea central:
 *
 * En lugar de almacenar un valor (como 5 o 'c'), un PUNTERO almacena
 * una DIRECCIÓN de memoria. Es como tener una nota que dice
 * “los datos están allí” en vez de sostener los datos directamente.
 *
...
/*
 * Cada variable se almacena en algún lugar de la RAM. Cada ubicación tiene
 * una DIRECCIÓN única, como el número de una casa en una calle. El OPERADOR
...
/*
 * Una vez definido un puntero, el asterisco `*` también desreferencia
 * y accede al valor almacenado en la dirección a la que apunta.
 * Por ello, se llama OPERADOR DE DESREFERENCIA (*).
 */
...
int main(void)
{
	...
	/*
	 * Se usa el operador dirección (&) para obtener su dirección.
	 * Además, `%p` es un formateador específico para imprimir
	 * direcciones de memoria en formato hexadecimal.
	 */
	...
	/*
	 * Declaramos un puntero a int e inicializamos a NULL por seguridad.
	 * Se añade un `p_` para indicar que es un puntero.
	 */
	int *p_score = NULL;
	printf("Valor inicial p_score (puntero seguro): %p\n", p_score);

	/* Ahora hacemos que el puntero apunte a 'score' asignándole su
	 * dirección.
	 */
	p_score = &score;
	printf("Valor de p_score tras la asignación: %p (dirección de 'score')\n",
	       p_score);
	...
	/*
	 * Para obtener el valor almacenado en la dirección que almacena
	 * un puntero se DESREFERENCIA usando *.
	 */
	...
	/*
	 * "Vete a la dirección almacenada en el puntero `p_score` y escribe
	 * allí el valor 100".
	 */
