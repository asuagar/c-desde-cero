/*
 * 11_punteros_y_vectores.c - Parte 2, Lección 11: Punteros y vectores
 *
 * Esta lección explica la relación entre punteros y arrays: cómo el nombre
 * del array se usa como dirección del primer elemento y cómo aplicar
 * ARITMÉTICA DE PUNTEROS de forma segura.
 *
 * Fecha: 15-10-2025
 * Autores:
 ...
 * =============================================================================
 *
 * En la lección anterior, vimos que un PUNTERO almacena una dirección
 * de memoria. Aquí exploramos una de las relaciones más importantes
 * del lenguaje C: la conexión entre punteros y vectores.
 *
 * LA GRAN REVELACIÓN:
 *
 * El nombre de un vector es el puntero de su primer elemento.
 * Esto significa que el nombre de un vector es una dirección de memoria.
 * Por eso, no hace falta un operador `&` cuando se usa `scanf` con una
 * cadena. ¡El propio nombre de la cadena proporciona la dirección de
 * memoria!
 *
 * Esta relación habilita la ARITMÉTICA DE PUNTEROS: movernos entre
 * elementos de un vector sumando un desplazamiento en unidades del tipo
 * de dato apuntado.
 */
...
int main(void)
{
	...
	printf("--- Parte 1: El nombre de un vector es un puntero ---\n");

	/*
	 * Vamos a demostrar que el nombre del vector `grades` se trata de
	 * una dirección. Para ello, imprimimos el valor de `grades` y la
	...
	printf("--- Parte 2: Aritmética de Punteros ---\n");
	...
	/*
	 * Ahora vamos a por la magia. ¿Qué sucede cuando se le suma 1 a un
	 * puntero? La ARITMÉTICA DE PUNTEROS es inteligente. Al sumar 1 a
	 * un puntero tipo int, mueve una unidad la dirección. La unidad es
	 * el tamaño de UN ELEMENTO del vector. Eso quiere decir que el
	 * puntero se mueve `sizeof(int)` bytes. En resumen, `*(p_grades + 1)`
	 * significa "vete a la dirección del primer elemento del vector,
	 * muévela un tamaño igual al tipo `int` y finalmente haz una
	 * desreferencia".
	 */
	...
	printf("--- Parte 3: Equivalencia de p[i] y *(p + i) ---\n");

	/*
	 * El lenguaje C garantiza que para un vector p y un índice i,
	 * la expresión p[i] es LO MISMO que *(p + i). La notación con los
	 * corchetes [] es más breve y fácil de usar. A esto se le llama
	 * "azúcar sintáctico" y hace la aritmética de punteros más amigable.
	 */
	...
	printf("--- Parte 4: Recorrer un vector con un puntero ---\n");
	printf("Grades: ");
 ...
 *    una función. La función no hace una copia local del vector, sino
 *    que trabaja directamente con las direcciones de memoria.
 */
