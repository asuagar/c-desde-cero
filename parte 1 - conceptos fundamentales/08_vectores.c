/*
 * 08_vectores.c - Parte 1, Lección 8: Vectores (Arrays)
 *
 ...
 * =============================================================================
 *
 * Hasta ahora, hemos usado variables que solo almacenan un valor a la vez.
 * Por ejemplo: `int score = 100;`. Pero ¿qué ocurre si queremos guardar las
 * notas de una clase de 30 alumnos?
 *
 * Declarar 30 variables diferentes (`score1`, `score2`, etc.) sería ineficiente.
 * Un VECTOR es una estructura que almacena una colección de elementos del
 * MISMO tipo y de tamaño fijo. Imagina una fila de cajas numeradas, donde cada
 * caja contiene un valor del mismo tipo.
 */
...
int main(void)
{
	 ...
	 *
	 * Ejemplo: un vector `high_scores` que puede almacenar 5 enteros.
	 */
	int high_scores[5];	/* Vector de 5 enteros. */

	/*
	 * ACCESO A ELEMENTOS:
	 * Se accede a cada elemento usando un ÍNDICE entre corchetes.
	 * C usa INDEXACIÓN DESDE CERO (ZERO-BASED INDEXING): el primer
	 * elemento está en el índice 0, el segundo en 1, y así sucesivamente.
	 * Para un vector de tamaño 5, los índices válidos son 0, 1, 2, 3 y 4.
	 *
	 * Acceder a un índice fuera de ese rango (por ejemplo, `high_scores[5]`)
	 * produce COMPORTAMIENTO INDEFINIDO, una causa común de errores.
	 */
	...
	high_scores[2] = 875;
	high_scores[3] = 820;
	high_scores[4] = 799;	/* Asigna 799 al quinto (último) elemento */
	 ...
	 *
	 * Si se proporciona una lista de inicialización, el compilador puede
	 * deducir automáticamente el tamaño dejando los corchetes vacíos.
	 */
	 ...
	 * con bucles. Un bucle `for` es ideal para recorrer un vector y
	...
	/*
	 * Haremos un bucle desde 0 hasta el último elemento, sin incluirlo.
	 * El contador `i` se usa como índice del vector.
	 */
	for (int i = 0; i < 5; i++) {
		printf("  Día %d: %.1f\n", i + 1, temperatures[i]);
	}
 ...
 * Más adelante aprenderás formas más seguras de manejar tamaños de vectores.
 */
