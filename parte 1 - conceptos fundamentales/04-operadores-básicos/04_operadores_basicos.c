 ...
 * @brief Parte 1, Lección 4: Operadores básicos
 * @details
 ...
 * sobre uno o más valores o variables (llamados OPERANDOS). Por ejemplo, en `5 + 3`,
 * el `+` es el operador y `5` y `3` son los operandos.
 *
...
int main(void) 
{
     ...
     * La parte fraccionaria se descarta (truncada). Esto se llama DIVISIÓN ENTERA.
     */

    printf("División entera (a / b) = %d\n", a / b); // 10 / 3 es 3, no 3.333
    ...
    printf("Módulo (a %% b) = %d\n", a % b); // 10 dividido entre 3 deja resto 1.
    printf("\n");
    ...
    // Un solo `=` es asignación. Doble `==` es comparación.
    // Es un error muy común entre principiantes.
    printf("¿a == 10? %d\n", a == 10); // Verdadero (1)
    printf("¿a == b?  %d\n", a == b);  // Falso (0)
    ...
    int is_player_alive = 1; // 1 es verdadero
    int has_key = 0;         // 0 es falso
    printf("Jugador vivo: %d, Jugador tiene llave: %d\n\n",
           is_player_alive, has_key);

    // `&&` (Y lógico) — verdadero solo si ambos operandos son verdaderos.
    ...
    // `!` (NO lógico) — invierte el valor lógico del operando.
    ...
    /*
     * Igual que en matemáticas (PEMDAS/BODMAS), los operadores en C tienen
     * un orden de evaluación predeterminado llamado PRECEDENCIA DE OPERADORES.
     * Por ejemplo, `*` y `/` se evalúan antes que `+` y `-`.
     */

    int result1 = 5 + 2 * 10;             // 2*10 primero, luego +5.
    printf("5 + 2 * 10 = %d\n", result1); // Muestra 25, no 70.
    ...
    int result2 = (5 + 2) * 10;             // (5+2) primero.
    printf("(5 + 2) * 10 = %d\n", result2); // Muestra 70.
