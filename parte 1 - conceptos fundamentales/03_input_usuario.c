/*
 * 03_input_usuario.c - Lección 3: Entrada del Usuario
 *
...
int main(void)
{
	 ...
	 *
	 * - `&user_age`: el operador `&` es el OPERADOR DIRECCIÓN DE MEMORIA.
	 ...
	 * Con `&user_age`, `scanf` recibe la *dirección* de la caja donde
	 * almacenar el dato introducido por el teclado.
	 */
	scanf("%d", &user_age);
	 ...
	 * Solo necesitamos cambiar el tipo de variable y el especificador
	 * de formato.
	 *
	 * IMPORTANTE: en `scanf`, el especificador de formato para un `double`
	 * es `%lf` (que significa “long float”). Esto es DIFERENTE de `printf`,
	...
	/*
	 * `scanf` a veces puede comportarse de manera inesperada debido a cómo
	 * maneja los espacios en blanco (como espacios, tabuladores y saltos
	 * de línea).
	 *
	 ...
	 * en la cadena de formato: `" %c"`. Ese espacio inicial le indica a
	 * `scanf` que ignore todos los caracteres en blanco previos antes de
	 * leer el carácter. Es un truco fundamental.
	 */
 ...
 * =============================================================================
 *
 * En esta lección aprendiste los fundamentos para hacer tus programas
 * interactivos.
 *
 * Puntos clave:
 * - La función `scanf()` lee entrada formateada desde el teclado.
 * - `scanf()` requiere el OPERADOR DE DIRECCIÓN `&` antes del nombre de la
 *   variable para saber *dónde* almacenar la entrada.
 * - Los especificadores de formato de `scanf()` pueden ser diferentes de
 *   `printf()`.
 *   - Para `double`: usa `%lf` con `scanf`, pero `%f` con `printf`.
 * - `scanf()` puede dejar caracteres de salto de línea en el búfer de entrada,
 *   lo que puede causar problemas. El truco de `" %c"` (con un espacio al
 *   inicio) se utiliza para evitarlo al leer caracteres.
 */
